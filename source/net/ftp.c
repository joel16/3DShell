...
static int ftp_set_socket_options(int fd)
{
	...
	rc = setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &sock_buffersize, sizeof(sock_buffersize));
...
static ssize_t ftp_session_write_file(ftp_session_t *session)
{
	...
	rc = fwrite(session->buffer + session->bufferpos, 1, session->buffersize - session->bufferpos, session->fp);
 ...
 */
static ssize_t ftp_send_response_buffer(ftp_session_t *session, const char *buffer, size_t len)
{
	ssize_t rc, to_send;
 ...
 *  returns bytes sent to peer
 */
static ssize_t ftp_send_response(ftp_session_t *session, int code, const char *fmt, ...)
{
...
static void ftp_session_new(int listen_fd)
{
	...
	/* reverse dns lookup */
	rc = getnameinfo((struct sockaddr*)&addr, addrlen, host, sizeof(host), serv, sizeof(serv), 0);
...
static int ftp_session_connect(ftp_session_t *session)
{
	...
	/* connect to peer */
	rc = connect(session->data_fd, (struct sockaddr*)&session->peer_addr, sizeof(session->peer_addr));
...
static void ftp_session_read_command(ftp_session_t *session, int events)
{
	...
	else
	{
		...
		while(true)
		{
			...
			/* look up the command */
			key.name = buffer;
			command = bsearch(&key, ftp_commands, num_ftp_commands, sizeof(ftp_command_t), ftp_command_cmp);
...
#define GiB (1024.0*MiB)
	...
	if (rc == 0)
	{
		...
		if (bytes_free < 1000.0)
			snprintf(buffer, sizeof(buffer), "%.0lfB", bytes_free);
...
static loop_status_t list_transfer(ftp_session_t *session)
{
	...
	if(session->bufferpos == session->buffersize)
	{
		...
		else
		{
			...
			{
				/* copy to the session buffer to send */
				session->buffersize = sprintf(session->buffer,
				...
				if(tm != NULL)
				{
					...
					if(session->timestamp > mtime && session->timestamp - mtime < (60*60*24*365/2))
						fmt = "%b %e %H:%M ";
					session->buffersize += strftime(session->buffer + session->buffersize, sizeof(session->buffer) - session->buffersize, fmt, tm);
				}
				else
					session->buffersize += sprintf(session->buffer + session->buffersize, "Jan 1 1970 ");
	...
	rc = send(session->data_fd, session->buffer + session->bufferpos, session->buffersize - session->bufferpos, 0);
	if(rc <= 0)
	{
		/* error sending data */
		if(rc < 0)
 ...
 *  @returns failure
 */
static int ftp_xfer_file(ftp_session_t *session, const char *args, xfer_file_mode_t mode)
{
	int rc;
 ...
 *  @returns failure
 */
static int ftp_xfer_dir(ftp_session_t *session, const char *args, xfer_dir_mode_t mode, bool workaround)
{
	ssize_t     rc;
	size_t      len;
	struct stat st;
		...
		if(session->dp == NULL)
		{
			...
			else
			{
				...
				if(buffer != NULL)
				{
					/* copy to the session buffer to send */
					session->buffersize =
					sprintf(session->buffer, "-rwxrwxrwx 1 3DS 3DS %lld Jan 1 1970 ", (signed long long)st.st_size);
					if(session->buffersize + len + 2 > sizeof(session->buffer))
					{
						/* buffer will overflow */
						free(buffer);
...
FTP_DECLARE(PASV)
{
	...
	/* bind to the port */
	rc = bind(session->pasv_fd, (struct sockaddr*)&session->pasv_addr, sizeof(session->pasv_addr));
	if(rc != 0)
	{
		ftp_session_close_pasv(session);
		return ftp_send_response(session, 451, "\r\n");
...
#ifndef _3DS
	{
		...
		rc = getsockname(session->pasv_fd, (struct sockaddr*)&session->pasv_addr, &addrlen);
		if(rc != 0)
		{
			ftp_session_close_pasv(session);
			return ftp_send_response(session, 451, "\r\n");
